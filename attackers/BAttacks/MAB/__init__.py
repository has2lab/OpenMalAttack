...
#                  random_sample=False,
#                  maxturns=10):
#         self.register_env(clsf, id, self.entry_point, random_sample, maxturns, sha256list)
#         self.test_model_env = gym.make(id)
#         self.test_model_env = wrappers.RecordVideo(self.test_model_env, video_folder=self.log_dir)
...
#         evasion_history = {}
#         test_model_agent = self.agent(self.test_model_env.action_space, self.test_model_env.observation_space.shape[0],
...
#             chosen_actions = {"remove_signature": False,
#                               "remove_debug": False,
#                               "break_optional_header_checksum": False}
            
#             while True:
#                 action = test_model_agent.choose_action(ob, chosen_actions)
...
#             pbar.set_postfix(label=((done and reward >= 10.0)==False))
#             pbar.update()
...
#         evasion_rate = (test_model_evasions / (self.test_episode_count - self.test_model_env.benign_count)) * 100
#         mean_action_count = np.mean(self.test_model_env.get_episode_lengths())
