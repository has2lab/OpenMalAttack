...
#   def __init__(self, sha256list: List[PathLike], model: Classifier, random_sample=False, maxturns=100, output_path='data/output/rl/evaded'):
#     self.bandit = Bandit()
...
#         score = self.model.predict_proba(self.model.extract(bytez))
...
#         # new_sha256 = calc_sha256(self.bytez)
#         # save_evaded_sample(self.output_path, new_sha256, self.bytez)
...
#     while True:  # 直到取到恶意样本为止
#         self.sample = self.samples_manager.get_next_sample()
#         if self.sample is None:
#             workings = self.samples_manager.get_count_with_status(SAMPLE_STATUS_WORKING)
#             if len(workings) == 0:
#                 remains = [x for x in self.samples_manager.get_samples_with_status(SAMPLE_STATUS_PENDING) if x.pull_count < self.maxturns]
#                 if len(remains) == 0:
#                     print(f'###### All samples are pulled {self.maxturn} times!')
#                 break
...
#         self.original_result = self.model.predict_proba(self.model.extract(self.bytez))
#         if self.original_result >= self.clsf_threshold:
#           break
#     return self.sample, basename(self.sample.path)
