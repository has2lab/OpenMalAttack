...
class Vocab:
    ...
    def __getitem__(self, item: str):
        ...
        if item in self.token_2_index.keys():
            ...
            if self.unk_token is not None:
                ...
                raise KeyError("{} is not in the dict, but self.unk_token is None".format(item))
    ...
    def create_vocabulary(self, freq_counter: Counter):
        ...
        for f_name, count in tqdm(freq_counter.most_common(self.max_vocab_size), desc="creating vocab ... "):
            if f_name in index_2_token:
                print("trying to add {} to the vocabulary dict, but it already exists !!!".format(f_name))
                continue
            else:
    ...
    def load_freq_counter_from_file(file_path: str, min_freq: int):
        ...
        with open(file_path, 'r') as f:
            for line in tqdm(f, desc="Load frequency list from file ... "):
                line = json.loads(line)
                f_name = line["f_name"]
                count = int(line["count"])
                
                assert f_name not in freq_dict, "trying to add {} to the vocabulary dict, but it already exists !!!"
                if count < min_freq:
                    print(line, "break")
                    break
...
if __name__ == '__main__':
    vocab = Vocab(freq_file="../processed_dataset/training_external_function_name_vocab.jsonl", max_vocab_size=50)
