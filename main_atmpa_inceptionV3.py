...
from attackers import ATMPA_Attacker
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="ATMPA attack on InceptionV3")
    parser.add_argument("--fpr", type=str, default="100", help="100 or 1000")
    args = parser.parse_args()
    
    fpr_type = "100fpr" if args.fpr == "100" else "1000fpr"
    
    attacker = ATMPA_Attacker()
    clsf = InceptionV3BytesWrapper(threshold_type=fpr_type)
    eval_atmpa_inception = Evaler(attacker=attacker, clsf=clsf)
    eval_atmpa_inception()
