...
class Sample:
    ...
    def inc_seq_cur_y(self):
        try:
            ...
            logger_rew.error('list_applied_arm: %s' % self.get_applied_actions())
            logger_rew.error('seq_cur_x: %d' % self.seq_cur_x)
            logger_rew.error(f'status: {self.status}')
            logger_rew.error(f'scan_status: {self.scan_status}')
            raise e# FIXME
    ...
    def replay_action_subset(self):     # For MAB Rewriter
        ...
        for arm in self.current_applied_arm_subset:
            if arm:
                ...
                # if 'minimizer_output' in os.path.dirname(input_path):  # FIXME: why delete?
