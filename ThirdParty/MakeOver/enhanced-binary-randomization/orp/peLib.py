...
# This file is part of enhanced-binary-diversification https://github.com/pwwl/enhanced-binary-diversification

# Additionally modified by wzy
...
def read_pe(pe_path=None, pe_data=None, remove_rubbish=True):
    """
    Given the path of the PE, load it using PE file, 
    and remove any rubbish appended to the end.
    """
    assert(pe_path!=None or pe_data!=None)
    if pe_path:
...
def _get_reloc_entries(disp_state):
    if not hasattr(disp_state.pe, 'DIRECTORY_ENTRY_BASERELOC'):
        return None
    reloc_entries = []
...
class AdjustPE:
   ...
   # Adjust IMAGE_FILE_HEADER
    ...
    def _adjust_reloc(self, moving_regions, reloc_entries, data_hash, disp_iter=1):
        """
        Adjust relocation entries in .reloc section
        moving_regions: (move start va, moved size, ropf inject va)
        reloc_entries: (entry data, rva to be reloc, entry type), in fact, only data and rva are needed, type is contained in data
        """
        ...
        if disp_iter==1:
            ...
            ropf_va = self.peinfo.getImageBase() + self.peinfo.getRVA(-1)
        ...
        # after being patched, the reloc table must be modified accordingly
        new_reloc_blocks = {}
        # print(reloc_blocks)
        for b_base in sorted(reloc_blocks.keys()):
            for entry in reloc_blocks[b_base]:
                ...
                # if reloc_addr in the instruction that will be moved, return (mov_start, mov_end), else return (0, 0)
        ...
        reloc_file = '/tmp/{}_reloc.dat'.format(data_hash) if os.name == 'posix' else '{}_reloc.dat'.format(data_hash)
        with open(reloc_file, 'wb') as f:
            f.write(new_reloc_data)

    def update_displacement(self, disp_state, data_hash='', size_increasement_budget=4096, disp_iter=1, DEBUG=False):
        """
        ...
        if disp_iter>1:
            ...
            dbin = ''
            instrs, semnop_bins, jmp_bin = disp_state.moving_instrs[-1]
            for ins in instrs:
                dbin += ins.bytes
            ...
            if reloc_entries is not None:
                self._adjust_reloc(moving_regions, reloc_entries, data_hash, disp_iter)
            
            va = moving_regions[-1][2]
            self.pe.set_bytes_at_rva(va-self.peinfo.getImageBase(), bytes(moving_code))
            return

        # Mahmood: simple hack to be compatible with Koo's
        # and Polychronakis' code
        moving_regions = disp_state.moving_regions
        # moving_code is the code in the .ropf section
        if size_increasement_budget>len(disp_state.get_dbin()):
            moving_code = disp_state.get_dbin() + '\x00'*(size_increasement_budget-len(disp_state.get_dbin()))
        else:
            moving_code = disp_state.get_dbin()
        # print([hex(i) for i in bytearray(moving_code)])
        ...
        reloc_entries = _get_reloc_entries(disp_state)
        if reloc_entries is not None:
            self._adjust_reloc(moving_regions, reloc_entries, data_hash)
