...
def classify_functions(functions):
  ...
  # imported functions=-1 / unclassified=-2 / 
  # code_refs_from is the set of addresses that to be called by the function
  level = 0
  ...
  while len(processed) < len(functions):
    ...
    for func in filter(lambda x: x.level == -2, functions.itervalues()):
      # print(func.level)
      # 第一轮处理哪些没有call的，随后处理那些只call了已经被处理过的函数的函数
      if func.code_refs_from <= processed:
        func.level = level
        curr_processed.add(func.addr)
    
    # 这个if成立后，如果func.ftype不为none，也可以加进去
    if curr_processed <= processed: # subset of processed
      ...
      for func in filter(lambda x: x.level == -2, functions.itervalues()):
        # func.ftype, e.g., int __thiscall(CWnd *this)
        if func.ftype:
          func.level = level
          curr_processed.add(func.addr)
